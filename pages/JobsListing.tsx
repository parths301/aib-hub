...
const JobsListing: React.FC = () => {
  ...
  const filteredJobs = useMemo(() => {
    if (!selectedCity) return jobs;
    return jobs.filter(job => job.city === selectedCity);
  }, [jobs, selectedCity]);

  if (loading) {
    return <div className="p-24 text-center text-sm font-black uppercase tracking-widest">Loading Opportunities...</div>;
  }
